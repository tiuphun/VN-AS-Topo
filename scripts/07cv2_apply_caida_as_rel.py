"""
07cv2_apply_caida_as_rel.py
============================
Classifies VN domestic edges (v2) using CAIDA's AS Relationship dataset.

Identical logic to 07c_apply_caida_as_rel.py, but reads from
vn_bgp_edges_v2.csv and writes to vn_edges_classified_v2.csv.

Input:
  - data/processed/vn_bgp_edges_v2.csv (from 07v2)
  - data/raw/20260101.as-rel2.txt.bz2 (or any *as-rel2* file)

Output:
  - data/processed/vn_edges_classified_v2.csv
"""
...
def classify_edges_v2():
    base_dir = os.path.join(os.path.dirname(__file__), '..')
    processed_dir = os.path.join(base_dir, 'data', 'processed')
    raw_dir = os.path.join(base_dir, 'data', 'raw')

    # ── Load v2 edges ────────────────────────────────────────────
    edges_path = os.path.join(processed_dir, 'vn_bgp_edges_v2.csv')
    if not os.path.exists(edges_path):
        print("ERROR: vn_bgp_edges_v2.csv not found. Run 07v2_parse_routeviews.py first.")
        return

    df = pd.read_csv(edges_path)
    print(f"Loaded {len(df)} v2 edges to classify.\n")
    ...
    # Show VNIX-related edges specifically
    vnix_edges = df[df['via_vnix'] == True]
    if len(vnix_edges) > 0:
        print(f"\n  VNIX-facilitated edges: {len(vnix_edges)}")
        vnix_counts = vnix_edges['relationship'].value_counts()
        for rel_type, count in vnix_counts.items():
            print(f"    {rel_type}: {count}")

    # ── Save ─────────────────────────────────────────────────────
    output_path = os.path.join(processed_dir, 'vn_edges_classified_v2.csv')
    ...
    if len(classified) > 0:
        ...
        for _, r in classified.iterrows():
            vnix_tag = " [via VNIX]" if r.get('via_vnix', False) else ""
            if r['relationship'] == 'p2c':
                print(f"  AS{int(r['provider_asn'])} → AS{int(r['customer_asn'])} (provider→customer){vnix_tag}")
            else:
                print(f"  AS{int(r['asn1'])} ↔ AS{int(r['asn2'])} (peer-to-peer){vnix_tag}")
